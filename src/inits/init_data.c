...
/*   Updated: 2024/10/11 17:27:23 by lmoran           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	init_mlx(t_mlx *game)
{
	game->mlx_p = mlx_init();
	if (!game->mlx_p)
		full_exit(game, "MLX INIT error");
	game->win = mlx_new_window(game->mlx_p, S_W, S_H, "cub3D");
	if (!game->win)
		full_exit(game, "WIN INIT error");
	game->tex_pix = 0;
	game->img_size = 64;
...
void	parser(t_mlx *mlx, t_data *data, int fd)
{
	...
	if (check_file(data) != 4)
		return (full_exit(mlx, "Parsing error"));
	ft_printf("%sINIT_DATA SUCCESS%s\n", green, rst);
}

void	init_data(t_mlx *mlx, t_data *data, int fd)
{
	data->player_dir = 0;
	data->exit = 0;
	data->north = 0;
...
void	init_player(t_mlx *mlx, t_player *ply)
{
	if (get_player_pos(&mlx->data) == FAIL)
		full_exit(mlx, "Error with player character in the map");
...
void	img_thing(t_mlx *mlx, t_image *img, char *path)
{
	...
	img->size_line = 0;
	img->endian = 0;
	img->img = mlx_xpm_file_to_image(mlx->mlx_p, path, &mlx->img_size,
			&mlx->img_size);
	if (!img->img)
		full_exit(mlx, "IMG malloc error");
...
int	*my_xpm_to_file(t_mlx *game, char *path)
{
	...
	buf = my_malloc(&game->garbage, 1, sizeof *buf * game->img_size
			* game->img_size, SINGLE);
	if (!buf)
		mlx_destroy_image(game->mlx_p, img.img);
	if (*game->garbage_freed == 1)
		full_exit(game, "xpm to file error");
	y = 0;
